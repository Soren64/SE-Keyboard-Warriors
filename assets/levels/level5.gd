...
@onready var prompt_txt4 = $EnemyContainer/Enemy4/RichTextLabel
...
@onready var vocabQ4 = $EnemyContainer/Enemy4/RichTextLabel2
@onready var choices4 = $EnemyContainer/Enemy4/Choices
			...
			if key_typed == next_char:
				...
				if current_letter_index == prompt.length():
					...
					if is_instance_valid(prompt_txt) == true:
						...
						prompt_txt3.show()
					elif is_instance_valid(prompt_txt3) == true:
						prompt_txt4.show()
				if enemy_count == 0:
					CompletedGame.show()
...
func save_game():
	...
	save_file.store_string("res://assets/levels/level5.tscn")
	save_file.close()
...
func _on_timer_timeout() -> void:
	...
	if enemyTurn == true:
		...
		&& is_instance_valid(vocabQ3) == true && is_instance_valid(choices3) == true:
			...
			choices3.show()
			prompt_txt3.hide()
		elif is_instance_valid(vocabQ3) == false && is_instance_valid(choices3) == false \
		&& is_instance_valid(vocabQ4) == true && is_instance_valid(choices4) == true:
			vocabQ4.show()
			choices4.show()
			prompt_txt4.hide()
		else:
...
func _on_answer_j_pressed() -> void:
	playerHealth -= 1
...
func _on_answer_k_pressed() -> void:
	if is_instance_valid(vocabQ) == false && is_instance_valid(choices) == false \
	&& is_instance_valid(vocabQ2) == true && is_instance_valid(choices2) == true:
		vocabQ2.hide()
		choices2.hide()
...
func _on_answer_l_pressed() -> void:
	playerHealth -= 1
